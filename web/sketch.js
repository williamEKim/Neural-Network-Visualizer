...
function paintAt(col, row) {
  ...
  for (let dr = -radius; dr <= radius; dr++) {
    for (let dc = -radius; dc <= radius; dc++) {
      ...
      const idx = r * 28 + c;
      if (maskMode) {
        pixels[idx] = Math.max(0, pixels[idx] - strength * 0.8);
      } else {
...
function getEffectiveWeights(layerIdx, neuronIdx) {
  ...
  if (layerIdx === 2) {
    const w2 = wLayers[2].weights[neuronIdx]; // length 50
    const w1eff = wLayers[1].weights[0].map((_, j100) =>
      w2.reduce((sum, w, j50) => sum + w * wLayers[1].weights[j50][j100], 0)
    );
    return wLayers[0].weights[0].map((_, pixelIdx) =>
  ...
  if (layerIdx === 3) {
    ...
    const w1eff = wLayers[1].weights[0].map((_, j100) =>
      w2eff.reduce((sum, w, j50) => sum + w * wLayers[1].weights[j50][j100], 0)
    );
    return wLayers[0].weights[0].map((_, pixelIdx) =>
      w1eff.reduce((sum, w, j) => sum + w * wLayers[0].weights[j][pixelIdx], 0)
    );
...
function drawConnections(H, layout) {
    ...
    if (layerIdx === 0) {
      ...
      if (hoveredNeuron && weights) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            netCtx.fillStyle = isHovered ? `rgb(${r},${g},${b})` : `rgb(${r},${g},${b})`;
            netCtx.fill();
            ...
            if (isHovered) {
                ...
                const biasLayerIdx = layerIdx; // which weights.layers index?
